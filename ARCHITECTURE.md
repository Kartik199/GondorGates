...
`GET /admin/policies`, `POST /admin/policies`, `DELETE /admin/policies/{path}`. `PolicyResolver` checks Redis overrides before YAML on every request. `AdminAuthWebFilter` with `Authorization: Bearer` and constant-time comparison. `RedisPolicyStore` loads overrides from Redis at startup so they survive restarts. `PolicyStore` interface extracted — `PolicyResolver` and `AdminPolicyController` depend on the interface, not `RedisPolicyStore` directly (DIP). JSR-380 validation on `POST /admin/policies` body.
...
`springdoc-openapi-starter-webflux-ui` 2.8.6 added. `OpenApiConfig` declares the API title, version, relative server URL (`/`), and `bearerAuth` security scheme. `@Operation`, `@ApiResponse`, and `@SecurityRequirement` annotations on `AdminPolicyController`. `@Schema` annotations on `RateLimitPolicy`, `DimensionPolicy`, and `RateLimitDimension` — including `minimum: 1` on `capacity` and `refillRate` to match `@Positive` validation. `DELETE /admin/policies/{*path}` uses a Spring path variable (not Ant `**`) so the OpenAPI path renders as `/admin/policies/{path}` with a documented `@Parameter`. 204 response carries `content = @Content` to suppress the default body in the spec. `GondorGatesWebFilter` updated to bypass rate limiting for `/swagger-ui/**`, `/v3/api-docs/**`, and `/webjars/**`. `RedisPolicyStore.save` and `delete` map Redis failures to `503 Service Unavailable` instead of propagating as 500. Swagger UI live at `/swagger-ui.html`; spec at `/v3/api-docs` (JSON) and `/v3/api-docs.yaml`. The `docs.yml` GitHub Actions workflow extended with a `generate-spec` job (Redis service container, app startup, curl) that exports `openapi.yaml` to GitHub Pages on every push to main — permanent spec URL: `https://kartik199.github.io/GondorGates/openapi.yaml`.
